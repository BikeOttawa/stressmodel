...
(function (exports) {
  ...
  const winterRoadwayFactor = 1
  const monthabbrev = {'jan':0, 'feb':1, 'mar':2, 'apr':3, 'may':4, 'jun':5, 'jul':6, 'aug':7, 'sep':8, 'oct':9, 'nov':10, 'dec':11}
  ...
  function bikingPermitted (way) {
    if (HasTag(way, 'highway') || HasTag(way, 'bicycle')) {
      ...
      if (HasTagValue(way, 'seasonal', 'yes')) {
        for (let tag in way.tags) {
          if (tag.startsWith('access:conditional')) {
            let v = way.tags[tag]
            v = v.replace(/ /g, '').toLowerCase();
            if (v === 'no@(dec-mar)') {
              return { permitted: false, result: {lts: 0, message: ['Cycling not permitted due to seasonal=\'yes\' tag.'], rule: 'wp7' } }      
            }
            else {
              if (v.substr(0, 3) === 'no@') {
